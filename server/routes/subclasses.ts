...
subclasses.get("/:subClassName",
  validator('param', (value, c) => {
    const { subClassName } = value;

    if (!subClassName || typeof subClassName !== 'string' || !subClassesKeys.includes(subClassName)) {
      return c.text(`Class name invalid. Valid subclasses: ${subClassesKeys.join(", ")}`, 400)
    }
...
subclasses.get("/by-class/:className",
  validator('param', (value, c) => {
