...
import dnd, { BackgroundTypes, ClassesType, RacesType, SubracesType } from "@types";
...
export async function generateBackstory(character: {
    ...
    const systemInstruction = "Eres un experto en D&D 5e que responde basándose únicamente en los datos proporcionados."
    ...
    ESTADÍSTICAS:
    ${Object.entries(character.stats).map(([stat, value]) => `${stat}: ${value}`).join(', ')}
